...
public:
	...
	void SetRenderTarget(const DX11GPUTexture2D& texture);
	...
	void SetShaderResource(const GPUShaderType shader_type, const DX11GPUTexture2D& texture, uint32_t slot = 0);
	...
	void ClearState();
	void ClearRenderTarget(const DX11GPUTexture2D& texture, const float color[4]);
	void ClearRenderTarget(const DX11GPUTextureView& render_target_view, const float color[4]);
	
	void ClearDepth(const DX11GPUTexture2D& texture, float value);
